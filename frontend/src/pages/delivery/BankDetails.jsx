...
const BankDetails = () => {
  ...
  const loadBankDetails = async () => {
    try {
      ...
      // Load bank details and delivery info
      const response = await balanceAPI.getDeliveryBoyBalance();
      
      if (response.success) {
        if (response.data.bankDetails && response.data.bankDetails.accountNumber) {
        ...
        setDeliveryInfo({
          ...
          commissionRate: 15, // Default to 15% (will be updated from API)
      ...
      try {
        ...
        if (commissionResponse.success) {
          setDeliveryInfo(prev => ({
            ...prev,
            commissionRate: commissionResponse.data.deliveryBoy
          }));
        }
  ...
  const handleUpdate = async () => {
    try {
      ...
      const response = await balanceAPI.updateDeliveryBoyBankDetails(bankDetails);
  ...
  const handleDelete = async () => {
    if (window.confirm("Are you sure you want to delete your bank details? This action cannot be undone.")) {
      try {
        setIsUpdating(true);
        const response = await balanceAPI.deleteDeliveryBoyBankDetails();
  ...
  return (
    <div className="px-4 sm:px-6 lg:px-8 pt-16 sm:pt-4 md:pt-2 pb-6 space-y-4 sm:space-y-6">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-1 gap-6">
          ...
          <div className="bg-white border border-gray-200 rounded-xl shadow-sm p-6">
            <div className="flex items-center justify-between mb-6">
              ...
              <div className="flex space-x-2">
                {!isEditing ? (
                  ...
                  >
                    <MdEdit size={14} />
                    <span>{bankDetails.accountNumber && bankDetails.accountNumber.trim() !== '' ? 'Edit' : 'Add'}</span>
                  </button>
                ) : (
                  <div className="flex space-x-2">
                    <button
              ...
              <div className="flex items-start space-x-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-blue-700 mt-1">
                    Your bank details are encrypted and stored securely. Used only for delivery payment processing.
                  </p>
                </div>
              </div>
            </div>
            ...
            {/* Delete Button */}
            {bankDetails.accountNumber && bankDetails.accountNumber.trim() !== '' && !isEditing && (
              <div className="mt-4 pt-4 border-t border-gray-200">
                <button
                  onClick={handleDelete}
                  disabled={isUpdating}
                  className="w-full px-4 py-2 text-sm text-red-600 border border-red-300 rounded-lg hover:bg-red-50 transition-all duration-300 disabled:opacity-50"
                >
                  {isUpdating ? 'Deleting...' : 'Delete Bank Details'}
                </button>
              </div>
            )}
          </div>
