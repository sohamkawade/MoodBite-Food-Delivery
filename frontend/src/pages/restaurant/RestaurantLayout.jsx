...
const RestaurantLayout = () => {
  ...
  const { isRestaurantLoggedIn, restaurantUser, logout, isLoading: authLoading } = useRestaurantAuth();
  ...
  useEffect(() => {
    if (!authLoading && !isRestaurantLoggedIn) {
      navigate("/restaurant/login");
    }
  }, [authLoading, isRestaurantLoggedIn, navigate]);
  ...
  const menuItems = [
    { id: "profile", label: "Profile", icon: <MdPerson size={24} />, path: "/restaurant/profile" },
    { id: "dashboard", label: "Dashboard", icon: <MdDashboard size={24} />, path: "/restaurant/dashboard" },
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600">Loading restaurant panel...</p>
        </div>
      </div>
    );
  }
  ...
  <div className="flex flex-col backcolor">
      ...
      <div className="flex flex-1">
        ...
        <div className={`text-gray-800 border-r border-gray-200 flex-col ${sidebarCollapsed ? 'w-20' : 'w-64'}
          fixed left-0 top-16 bottom-0 z-50 backcolor transform transition-transform duration-500 ease-in-out will-change-transform md:sticky md:top-16 md:self-start md:h-[calc(100vh-4rem)] md:overflow-y-auto md:transform-none md:flex
        ...
        {/* Main Content */}
        <div className={`flex-1 flex flex-col overflow-hidden pt-16 md:pt-0 transition-all duration-500`}>
