...
import { MdArrowBack, MdCheckCircle, MdError, MdRestaurant } from "react-icons/md";
...
const RestaurantForgotPassword = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await restaurantAuthAPI.forgotPassword({ email });
  ...
  const handleVerifyOtp = async (e) => {
    ...
    try {
      await restaurantAuthAPI.verifyOTP(email, otpString);
  ...
  const handleResetPassword = async (e) => {
    ...
    try {
      const otpString = otp.join("");
      await restaurantAuthAPI.resetPassword(email, otpString, newPassword);
            ...
            <div className="text-center">
              ...
              <form onSubmit={handleVerifyOtp} className="space-y-4">
                <div className="flex justify-center space-x-2">
                  {otp.map((digit, index) => (
                    <input
                      ...
                      className="w-10 h-10 sm:w-12 sm:h-12 text-center text-lg sm:text-xl font-bold border-2 border-gray-300 rounded-lg focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-200"
                    />
                  ))}
                </div>
                ...
                <button
                  ...
                  className="w-full bg-blue-500 text-white py-2 px-4 rounded-lg hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transition-colors font-medium disabled:opacity-50 disabled:cursor-not-allowed"
                >
                ...
                <Link
                  to="/restaurant/login"
                  className="flex items-center justify-center w-full text-blue-600 hover:text-blue-700 font-medium py-2"
                >
        ...
        <div className="bg-white rounded-2xl shadow-xl p-4 sm:p-6 md:p-8 lg:p-6">
          <div className="text-center mb-5 sm:mb-6">
            <div className="flex justify-center mb-4">
              <div className="w-16 h-16 bg-blue-100 rounded-full flex items-center justify-center">
                <MdRestaurant size={32} className="text-blue-600" />
              </div>
            </div>
            ...
            <p className="text-gray-600 text-sm sm:text-sm">
              Enter your restaurant email address and we'll send you a secure reset link.
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-3 sm:space-y-4">
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="w-full bg-blue-500 text-white py-2 md:py-2 px-4 md:px-6 rounded-lg hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transition-colors font-semibold disabled:opacity-50 disabled:cursor-not-allowed text-sm md:text-sm"
            >
              {isLoading ? (
            ...
            <div className="text-center">
              <Link
                to="/restaurant/login"
                className="text-blue-500 hover:text-blue-600 text-xs sm:text-sm md:text-sm font-medium"
              >
